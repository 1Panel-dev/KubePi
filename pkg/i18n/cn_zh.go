...
var zhCNMapping = TextMapping{
	...
	"username or password error":                    "您输入的用户名或密码不正确，请重新输入",
	"Unauthorized":                                  "认证失败",
	"permission %s required":                        "权限不足：%s",
	...
	"no login user":                                 "用户未登录",
	"kubernetes api server %s connection timed out": "连接 Kubernetes API Server %s 超时，请检查 KubePi 服务所在环境到该地址的网络、安全组或防火墙配置",
	"can not verify kubernetes api server %s: %s":   "无法校验 Kubernetes API Server %s：%s",
}
