...
var enUSMapping = TextMapping{
	...
	"kubernetes api server %s connection timed out": "Connection to Kubernetes API Server %s timed out. Check the network, security group, or firewall from the KubePi service environment to this address.",
	"can not verify kubernetes api server %s: %s":   "Cannot verify Kubernetes API Server %s: %s",
}
