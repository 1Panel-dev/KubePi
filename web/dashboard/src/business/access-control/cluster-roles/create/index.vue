<template>
  <layout-content :header="$t('commons.button.create')" :back-to="{name: 'ClusterRoles'}" v-loading="loading">
...
import {createClusterRole} from "@/api/clusterroles"
...
export default {
  ...
  methods: {
    ...
    onCreate (data) {
      this.loading = true
      createClusterRole(this.cluster, data).then(() => {
        this.$message({
          type: "success",
