    ...
    <div v-if="showYaml === 'false'">
      ...
      <el-tabs style="margin-top: 30px;background-color: #141418;" type="border-card" v-model="activeName">
        ...
        <el-tab-pane label="Health Check" name="Health Check">
          <div :key="isRefresh">
            <ko-health-check ref="ko_health_readiness_check" :healthCheckParentObj="form.spec.jobTemplate.spec.template.spec.containers[currentContainerIndex]" style="margin-top=30px" health_check_type="Readiness Check" />
            <ko-health-check ref="ko_health_liveness_check" :healthCheckParentObj="form.spec.jobTemplate.spec.template.spec.containers[currentContainerIndex]" style="margin-top=30px" health_check_type="Liveness Check" />
            <ko-health-check ref="ko_health_startup_check" :healthCheckParentObj="form.spec.jobTemplate.spec.template.spec.containers[currentContainerIndex]" style="margin-top=30px" health_check_type="Startup Check" />
          </div>
        </el-tab-pane>
...
import { getCronJobByName, createCronJob } from "@/api/workloads"
