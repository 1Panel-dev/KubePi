<template>
  <layout-content :header="$t('commons.button.edit')" :back-to="{name: 'Mutatingwebhookconfigurations'}"
                  v-loading="loading">
...
import {updateMutatingwebhookconfiguration, getMutatingwebhookconfiguration} from "@/api/mutatingwebhookconfiguration";
...
export default {
  ...
  methods: {
    ...
    onSubmit() {
      ...
      this.loading = true
      updateMutatingwebhookconfiguration(this.cluster, this.name, data).then(() => {
        this.$message({
          type: "success",
