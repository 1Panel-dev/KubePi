<template>
  <div v-if="terminalOpened">
    <div style="position: relative;">
      <el-tabs class="TabsWithoutContant" style="min-height: 35px;background-color: #1f2224" type="border-card"
               v-model="currentTab" @tab-click="handleClick" closable @tab-remove="removeTab">
      ...
      <div class="tabButton">
        <el-button v-if="contentOpened" size="mini" icon="el-icon-arrow-down" @click="toggleContentOpened"
                   circle/>
        <el-button v-if="!contentOpened" size="mini" icon="el-icon-arrow-up" @click="toggleContentOpened"
                   circle/>
      </div>

      <div v-show="contentOpened">

        <div style="height:50px">
          <div class="terminalOption">
            <span class="spanClass">{{ $t('business.workload.container') }}</span>
            <el-select class="interval" @change="changeConditions(currentTerminal)" size="mini"
                       v-model="currentTerminal.container">
        ...
        <div style="height: 600px;overflow: scroll">
          <div v-for="item in terminalTabs" :key="item.key">
            <iframe v-show="item.show" :src="item.url"
                    style="width: 100%;height: 100%;min-height: 800px;border: 0"></iframe>
          </div>
        </div>
...
export default {
  ...
  watch: {
    terminalTabs: function (val) {
      if (val && val.length > 0) {
        // 第一个窗口打开容器
        if (val.length === 1) {
          this.openOrRefreshTerminalContainer()
        }
  ...
  methods: {
    openOrRefreshTerminalContainer() {
      this.setTerminalContainerHeight(document.body.clientHeight * 0.5)
    },
    closeTerminalContainer() {
      this.setTerminalContainerHeight(0)
    },
    setTerminalContainerHeight(height) {
      this.$store.commit("app/CHANGE_BOTTOM_HEIGHT", height)
      this.containerHeight = height
      // 计算content的大小
      this.terminalHeight = this.containerHeight - 65;
    },
    ...
    toggleContentOpened() {
      ...
      if (!this.contentOpened) {
        this.setTerminalContainerHeight(40)
      } else {
        this.openOrRefreshTerminalContainer()
      }
    },
