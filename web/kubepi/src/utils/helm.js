...
import { listSecretsWithLabelSelector } from "@/api/secrets"
import {
    createTarGzip
  } from "nanotar";
...
export async function downloadHelmReleases(clusters) {
    ...
    if(clusters && clusters.length>0){
     for (let i = 0; i < clusters.length; i++) {
       try{
        ...
        //获取对应secrets
        const secrets_res = await listSecretsWithLabelSelector(clusterName,"status=deployed,owner=helm");
        console.log(secrets_res)
        const secrets=secrets_res.items.filter(item => item.metadata.name.startsWith("sh.helm.release."))
        //加入文件到tgz
        await helmSecretToTgz(clusterName,secrets,tgz)
       }catch(e){
        console.log(e)
       }
     }
     if(tgz.length>0){
      ...
      $error("no found helm releases")
     }
    }

}
//加入secret中的加密文件到tgz
async function helmSecretToTgz(clusterName,secrets,tgz) {
    for (let ii = 0; ii < secrets.length; ii++) {
        ...
        for (let i = 0; i < chartJSONS.length; i++) {
            ...
            //加入加密后的chartjson数据到tgz
               await helmChartJSONToTgz(clusterName,secret.metadata.namespace,chartJSON,tgz)
            }
